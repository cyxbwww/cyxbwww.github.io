...
│   │   └── proxy.ts           // 网络请求代理
│   ├── plugins                // 构建插件
│   │   ├── unocss.ts          // 原子css框架unocss插件、配置
│   │   ├── unplugin.ts        // 自动导入组件、自动解析iconify图标、自动解析本地svg作为图标
...
│   │   ├── auto-imports.d.ts  // 自动导入的插件的类型声明
│   │   ├── axios.d.ts         // axios扩展的类型声明
...
├── pnpm-lock.yaml             // npm包管理器npm依赖锁定文件
├── README.md                  // 项目介绍文档
├── tsconfig.json              // TS配置
