...
那么我们是不是可以使用双重断言 `as any as Foo` 来将任何一个类型断言为任何一个类型呢？
...
在上面的例子中，如果直接使用 `cat as Fish` 肯定会报错，因为 `Cat` 和 `Fish` 互相不兼容。

但是如果使用双重断言，则可以打破「要使得 `A` 能够被断言为 `B`，只需要 `A` 兼容 `B` 或 `B` 兼容 `A` 即可」的限制，将任何一个类型断言为任何另一个类型。

如果你使用了这种双重断言，那么十有八九是非常错误的，它很可能会导致运行时错误。
...
而在前一个例子中，由于 `getChcheData('tom')` 是 `any` 类型，`any` 兼容 `Cat`，`Cat` 也兼容 `any`，所以
