...
`Promise.prototype.then()` 是为期约实例添加处理程序的主要方法。可以接收两个参数：`onResolved` 和 `onRejected`。两个参数可选，如果提供会在期约分别进入“兑现”和“拒绝”状态时执行。
...
传给 `then()` 的任何非函数类型的参数都会被静默忽略。如果只提供 `onRejected` 参数，那就要在 `onResolved` 参数的位置上传入 `undefined`。这样有助于避免在内存中创建多余的对象，对期待函数参数的类型系统也是一个交代。
